...
import { clearCart, removeCartItem, minusCartItem, plusCartItem } from '../redux/actions'
...
function Cart() {
    ...
    const pizzas = Object.keys(items).map(obj => items[obj].items[0])
            ...
            {totalCount ?
                <div className="cart">
                    ...
                    <div className="content__items">
                        {
                            pizzas.map(obj => <CartItem minusItem={minusItem} plusItem={plusItem} name={obj.name} size={obj.size} image={obj.image} id={obj.id} key={obj.id} totalPrice={items[obj.id].totalPrice} totalCount={items[obj.id].items.length} removeItem={removeItem} />)
                        }
